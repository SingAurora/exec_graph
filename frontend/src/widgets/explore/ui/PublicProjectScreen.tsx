...
import { adoptReviewedContributions, getCollaborationCallDetail, getPublicProjectDetail, listContributionSourceRecords, reviewContributionBatch, submitProjectContribution, type CollaborationCall, type CollaborationReviewBatch, type CollaborationSubmission, type ContributionSource, type ExploreProject } from '@/features/collaboration/api/client'
...
export function PublicProjectScreen() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const data = await getPublicProjectDetail(token, projectUuid)
      setProject(data.project)
      if (token) {
        const calls = await Promise.all(data.project.calls.map((call) => getCollaborationCallDetail(token, call.uuid)))
        setDetails(Object.fromEntries(calls.map((item) => [item.call.uuid, item])))
      } else {
        setDetails({})
      }
...
function ContributionCall({ call, submissions, token, isOwner, isAuthenticated, onChanged }: { call: CollaborationCall; submissions: CollaborationSubmission[]; token: string; isOwner: boolean; isAuthenticated: boolean; onChanged: () => Promise<void> }) {
  ...
  const [sourceRecordUuid, setSourceRecordId] = useState('')
  ...
  const submittedIDs = useMemo(() => submitted.map((item) => item.uuid), [submitted])
