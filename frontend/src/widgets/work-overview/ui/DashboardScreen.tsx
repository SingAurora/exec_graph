...
export function DashboardScreen() {
  ...
  const currentNodes = contracts
    .filter((contract) => currentIDs.has(contract.uuid) && !contract.completionRecordUuid)
  ...
  return (
    <section className={`min-w-0 border-b border-rail last:border-b-0 ${contracts.length === 0 ? 'hidden' : ''}`} aria-label={copy.title}>
      ...
      <div className="divide-y divide-rail">
        {contracts.length > 0 ? contracts.map((contract) => <QueueNode key={contract.uuid} contract={contract} project={projects.get(contract.projectUuid)} railClassName={copy.railClassName} />) : <p className="px-5 pb-5 text-sm text-graphite">{copy.empty}</p>}
      </div>
    </section>
  )
}
...
function QueueNode({ contract, project, railClassName }: { contract: ExecutionContract; project?: Project; railClassName: string }) {
  return (
    <Link to={`/contracts/${contract.uuid}?tab=completion`} className="group flex gap-4 px-5 py-4 transition hover:bg-shell/45 focus:outline-none focus-visible:shadow-focusline">
...
function ProjectCard({ project }: { project: Project }) {
  ...
  const completionRecords = useExecStore((state) => state.completionRecords)
  const branches = useExecStore((state) => state.branches)
  const projectContracts = contracts.filter((contract) => contract.projectUuid === project.uuid)
  const projectRecords = completionRecords.filter((record) => record.projectUuid === project.uuid && isAcceptedRecord(record))
  const isArchived = Boolean(project.archivedAt)
  const currentContract = contracts.find((contract) => contract.uuid === project.currentContractUuid)
  const activeBranches = branches.filter((branch) => branch.projectUuid === project.uuid && branch.currentContractUuid)
  const currentContractIDs = new Set([project.currentContractUuid ?? '', ...activeBranches.map((branch) => branch.currentContractUuid ?? '')])
  const pendingContracts = projectContracts.filter((contract) => currentContractIDs.has(contract.uuid) && !contract.completionRecordUuid).length
  const branchStatus = activeBranches.length > 0 ? `${activeBranches.length} 条路径推进中` : '可以开始下一项推进'

  return (
    <Link to={`/projects/${project.uuid}`} className="group grid min-w-0 gap-4 border-b border-rail px-5 py-5 last:border-b-0 transition hover:bg-shell/45 focus:outline-none focus-visible:shadow-focusline lg:grid-cols-[minmax(0,1fr)_220px_auto] lg:items-center">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
