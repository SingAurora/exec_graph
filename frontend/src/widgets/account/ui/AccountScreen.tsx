...
import { listCurrentUserContributions, type ContributionActivity } from '@/features/collaboration/api/client'
...
export function AccountScreen() {
  ...
  const selectTab = (tab: ProfileTab) => {
	...
	useEffect(() => {
		...
		listCurrentUserContributions(accessToken)
  ...
  return (
    <section>
      ...
      {activities.length > 0 ? <div className="divide-y divide-rail border-b border-rail bg-surface/45">{activities.map((activity) => {
        ...
        return <Link key={activity.submission.uuid} to={`/explore/projects/${activity.call.projectUuid}#call-${activity.call.uuid}`} className="grid gap-3 px-1 py-4 transition hover:bg-shell sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center sm:px-4 focus:outline-none focus-visible:shadow-focusline">
  ...
  return (
    <section>
      ...
      <div className="grid gap-4 pt-5 lg:grid-cols-2">
        {projects.length > 0 ? projects.map((project) => <PublicProjectCard key={project.uuid} project={project} records={publicCompleted.filter((record) => record.projectUuid === project.uuid).length} />) : (
    ...
    <section>
      ...
      <div className="border-b border-rail bg-surface/45">
        {records.length > 0 ? records.map((record, index) => (
          <Link key={record.uuid} to={`/contracts/${record.closingContractUuid}`} className="grid gap-3 border-t border-rail px-1 py-4 transition hover:bg-shell sm:grid-cols-[52px_minmax(0,1fr)_auto] sm:items-center sm:px-4 focus:outline-none focus-visible:shadow-focusline">
            <span className="font-mono text-xs font-semibold text-signal">{String(records.length - index).padStart(2, '0')}</span>
            <span className="min-w-0"><span className="block truncate text-sm font-semibold text-ink">{record.title}</span><span className="mt-1 block text-xs leading-5 text-graphite"><CheckCircle2 size={13} className="mr-1 inline text-moss" aria-hidden="true" />覆盖 {record.coveredContractUuids.length} 个推进节点 · {record.aiReviewVerdict === 'pass' ? '行动记录 · 用户确认' : '记录有缺口 · 用户保存'} · {new Intl.DateTimeFormat('zh-CN', { year: 'numeric', month: 'numeric', day: 'numeric' }).format(new Date(record.createdAt))}</span></span>
...
function PublicProjectCard({ project, records }: { project: Project; records: number }) {
  return (
    <Link to={`/projects/${project.uuid}`} className="group block border border-rail bg-surface/72 p-5 transition hover:border-graphite/40 hover:bg-shell focus:outline-none focus-visible:shadow-focusline">
