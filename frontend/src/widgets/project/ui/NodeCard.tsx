...
export function NodeCard({ node, compact = false, actionLabel = '查看节点' }: NodeCardProps) {
  const project = useExecStore((state) => state.projects.find((item) => item.uuid === node.projectUuid))
  const smartContract = useExecStore((state) => state.smartContracts.find((item) => item.uuid === node.smartContractUuid))

  return (
    <Link
      to={`/contracts/${node.uuid}`}
