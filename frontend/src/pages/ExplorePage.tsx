...
export function ExplorePage() {
  ...
  useEffect(() => {
    if (!token) {
      setProjects([])
      setNetwork(null)
      setError('登录后可以查看公开协作网络。')
      setLoading(false)
      return
    }
    let alive = true
    Promise.all([getExploreProjects(token), getExploreNetwork(token)]).then(([projectData, networkData]) => { if (alive) { setProjects(projectData.projects); setNetwork(networkData) } }).catch((reason: Error) => {
      if (!alive) return
      setError(reason.message === '请先登录' ? '登录会话已失效，请重新登录后查看公开协作网络。' : reason.message)
