...
import { getExploreProjects, type CollaborationCall, type ExploreProject } from '../lib/collaboration'
...
export function ExplorePage() {
  ...
  useEffect(() => {
    let alive = true
    getExploreProjects(token).then((data) => { if (alive) setProjects(data.projects) }).catch((reason: Error) => { if (alive) setError(reason.message) }).finally(() => { if (alive) setLoading(false) })
    return () => { alive = false }
  }, [token])
  ...
  return <div className="space-y-9">
    <section className="border-b border-rail pb-7"><div className="flex flex-wrap items-end justify-between gap-5"><div><div className="flex items-center gap-2 font-mono text-xs font-semibold uppercase text-signal"><Compass size={15} />协作探索</div><h1 className="mt-3 max-w-3xl font-display text-4xl font-semibold leading-tight text-ink">接住彼此的成果，把共同难题推进到底</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-graphite">每个开放缺口都带着冻结的目标、证据要求和下游影响。提交自己的已验收成果，或从材料开始一项新的贡献。</p></div><label className="relative block w-full sm:w-80"><Search size={16} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-graphite" /><input value={query} onChange={(event) => setQuery(event.target.value)} type="search" placeholder="搜索共同难题或开放缺口" className="h-10 w-full border border-rail bg-surface/72 pl-10 pr-3 text-sm outline-none transition placeholder:text-graphite/70 focus:border-signal focus:shadow-focusline" /></label></div></section>
