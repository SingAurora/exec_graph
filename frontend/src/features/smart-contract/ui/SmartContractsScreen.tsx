...
import { listSmartContractHistory, type SmartContractEvent } from '@/entities/smart-contract/api/client'
...
export function SmartContractsScreen({ compact = false }: { compact?: boolean }) {
  ...
  useEffect(() => {
    ...
    const loadEvents = async () => {
      try {
		const data = await listSmartContractHistory(accessToken)
  ...
  const onDelete = async (contractUuid: string) => {
    setMessage('')
    const result = await deleteSmartContract(contractUuid)
    if (!result.success) {
    ...
    <div className="space-y-8">
      ...
      <section className="space-y-5" aria-labelledby="contract-library-title">
        ...
        <div className={compact ? 'divide-y divide-rail border-y border-rail' : 'grid gap-4 xl:grid-cols-2'}>
          {visibleContracts.map((smartContract) => (
            <SmartContractCard key={smartContract.uuid} compact={compact} smartContract={smartContract} projectUuids={projects.filter((project) => project.contractRevisions.some((revision) => revision.uuid === project.activeContractRevisionUuid && revision.smartContractUuid === smartContract.uuid)).map((project) => project.uuid)} onDelete={onDelete} />
          ))}
        </div>
      </section>
      ...
      <section className="border-t border-rail pt-6" aria-labelledby="contract-history-title">
        ...
        <div className="mt-5 grid gap-3">
          {events.length === 0 ? <p className="text-sm text-graphite">新的自定义合约会在这里记录创建与删除。</p> : events.map((event) => <SmartContractEventRow key={event.uuid} event={event} />)}
        </div>
      </section>
    </div>
  )
...
function SmartContractCard({ compact = false, smartContract, projectUuids, onDelete }: { compact?: boolean; smartContract: SmartContractDefinition; projectUuids: string[]; onDelete: (contractUuid: string) => Promise<boolean> }) {
  const projects = useExecStore((state) => state.projects)
  const activeProjects = projects.filter((project) => projectUuids.includes(project.uuid))
  ...
  const remove = async () => {
    setIsDeleting(true)
    setDeleteMessage('')
    const deleted = await onDelete(smartContract.uuid)
  ...
  return (
    <article className={compact ? 'py-5 first:pt-0 last:pb-0' : 'rounded-md border border-rail bg-surface/72 p-5'}>
      ...
      <div className={`flex flex-wrap items-center gap-2 text-sm ${compact ? 'mt-3' : 'mt-5 border-t border-rail pt-4'}`}>
        ...
        {activeProjects.length > 0 ? activeProjects.map((project) => <Link key={project.uuid} to={`/projects/${project.uuid}`} className="font-semibold text-signal transition hover:text-ink focus:outline-none focus-visible:shadow-focusline">{project.title}</Link>) : <span className="text-graphite">尚未采用</span>}
