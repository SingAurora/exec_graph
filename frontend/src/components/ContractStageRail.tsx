    ...
    <div className="border-y border-rail bg-surface px-5 py-4">
      ...
      <div className="mt-4 flex items-start">
        {stages.map((item, index) => {
          ...
          const currentClass = stage === 'needs_supplement' ? 'border-clay bg-clay text-white' : 'border-signal bg-signal text-white'
          return (
            <div key={item.id} className="flex min-w-0 flex-1 items-center last:flex-none">
              <div className="min-w-0">
                ...
                <div className={`mt-2 whitespace-nowrap text-xs font-semibold ${current ? (stage === 'needs_supplement' ? 'text-clay' : 'text-signal') : reached ? 'text-moss' : 'text-graphite'}`}>{item.label}</div>
              </div>
