...
func (s *server) handleDailyWorkReview(w http.ResponseWriter, r *http.Request) {
	user, ok := s.requireUser(w, r)
	if !ok {
		return
	}
	...
	dateValue := strings.TrimSuffix(strings.TrimPrefix(r.URL.Path, "/api/work-overview/days/"), "/review")
	...
	defer cancel()
	days, err := s.loadDailyWorkDays(ctx, user.ID, date, date.AddDate(0, 0, 1))
	if err != nil {
	...
	key, err := s.loadWorkOverviewAIKey(ctx, user.ID)
	...
	review, err := s.saveDailyWorkReview(ctx, user.ID, date, key.snapshot(), modelOutput)
	if err != nil {
		writeError(w, http.StatusInternalServerError, "保存日结分析失败")
		return
	}
	_, _ = s.db.ExecContext(ctx, `UPDATE ai_api_keys SET last_used_at = NOW() WHERE id = ? AND user_id = ?`, key.ID, user.ID)
...
func (s *server) loadDailyWorkDays(ctx context.Context, userID uint64, start, end time.Time) ([]dailyWorkDayResponse, error) {
	...
	records, err := s.db.QueryContext(ctx, `
		SELECT r.id, r.project_id, p.title, r.closing_contract_id, r.title, r.summary, r.record_kind, r.created_at
...
func (s *server) loadDailyWorkReviews(ctx context.Context, userID uint64, start, end time.Time) (map[string]dailyWorkReviewResponse, error) {
	rows, err := s.db.QueryContext(ctx, `
		SELECT id, DATE_FORMAT(review_date, '%Y-%m-%d'), review_json, ai_config_json, created_at, updated_at
	...
	for rows.Next() {
		...
		if err := rows.Scan(&review.ID, &review.Date, &reviewJSON, &configJSON, &review.CreatedAt, &review.UpdatedAt); err != nil {
			return nil, err
		}
...
func (s *server) saveDailyWorkReview(ctx context.Context, userID uint64, date time.Time, config aiConfigSnapshot, output dailyWorkReviewModelOutput) (dailyWorkReviewResponse, error) {
	...
	err := s.db.QueryRowContext(ctx, `SELECT id, created_at FROM daily_work_reviews WHERE user_id = ? AND review_date = ?`, userID, dateValue).Scan(&reviewID, &createdAt)
	if errors.Is(err, sql.ErrNoRows) {
		var idErr error
	...
	_, err = s.db.ExecContext(ctx, `
		INSERT INTO daily_work_reviews (id, user_id, review_date, review_json, ai_config_json)
		VALUES (?, ?, ?, ?, ?)
