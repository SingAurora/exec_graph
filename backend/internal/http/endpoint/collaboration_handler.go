...
func (s *Server) handleProjectCollaborationCalls(w http.ResponseWriter, r *http.Request, userID uint64, projectID string) {
	...
	call, err := s.collaboration.CreateCall(r.Context(), applicationcollaboration.CreateCallInput{
...
func (s *Server) handleExploreProjects(w http.ResponseWriter, r *http.Request) {
	projects, err := s.collaboration.ListExplore(r.Context())
	if err != nil {
...
func (s *Server) handleExploreProject(w http.ResponseWriter, r *http.Request, projectID string) {
	project, err := s.collaboration.GetProject(r.Context(), strings.TrimSpace(projectID))
	if errors.Is(err, applicationcollaboration.ErrNotFound) {
...
func (s *Server) getCollaborationCall(w http.ResponseWriter, r *http.Request, callID string) {
	details, err := s.collaboration.GetCallDetails(r.Context(), callID)
	if errors.Is(err, applicationcollaboration.ErrNotFound) {
...
func (s *Server) createCollaborationSubmission(w http.ResponseWriter, r *http.Request, userID uint64, callID string) {
	...
	details, err := s.collaboration.Submit(r.Context(), applicationcollaboration.SubmitInput{
