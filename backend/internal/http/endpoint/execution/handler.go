...
func (h *Handler) CreateNode(w http.ResponseWriter, r *http.Request, userID uint64, projectID string) error {
	...
	result, err := h.project.CreateNode(r.Context(), applicationproject.CreateNodeInput{
		OwnerID: userID, ProjectID: projectID, Draft: request.Draft, DraftReview: request.DraftReview,
		DraftReviewVerdict: request.DraftReview.Verdict, DraftReviewKeyID: request.DraftReview.AIConfig.KeyID,
		Title: request.Title, VerifiableGoal: request.VerifiableGoal, AcceptanceCriteria: criteria,
		EvidenceRequirement: request.EvidenceRequirement, ParentContractID: request.ParentContractID,
		SourceContractIDs: request.SourceContractIDs, BranchID: request.BranchID, Fork: request.Fork,
		SupplementOfNodeID: request.SupplementOfNodeID, RetryOfContractID: request.RetryOfContractID,
		Closure: request.Closure, PlanningConversationID: request.PlanningConversationID,
	})
...
// LockNode confirms a reviewed node and writes its immutable completion record.
func (h *Handler) LockNode(w http.ResponseWriter, r *http.Request, userID uint64, projectID, nodeID string) error {
