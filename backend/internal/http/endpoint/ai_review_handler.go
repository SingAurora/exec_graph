...
func (key aiStoredKey) snapshot() aiConfigSnapshot {
	return aiConfigSnapshot{KeyID: key.UUID, Label: key.Label, Provider: key.Provider, Model: key.Model, BaseURL: key.BaseURL}
}
...
func (s *Server) reviewExecutionNode(w http.ResponseWriter, r *http.Request) {
	...
	result, err := s.reviews.Execute(ctx, `
	...
	if _, err := s.reviews.Execute(ctx, `UPDATE ai_api_keys SET last_used_at = NOW() WHERE uuid = ? AND user_id = ?`, key.UUID, user.ID); err != nil {
...
func (s *Server) loadNodeCompletionReviewRequest(ctx context.Context, userID uint64, input reviewExecutionNodeRequest) (reviewExecutionNodeRequest, string, error) {
	...
	var stage string
	if err := s.reviews.Row(ctx, `SELECT stage FROM execution_contracts WHERE uuid = ?`, input.NodeID).Scan(&stage); err != nil {
		return request, "", err
	}
	if stage != "frozen" {
	...
	if branchID.Valid {
		...
		if err := s.reviews.Row(ctx, `SELECT current_contract_id FROM execution_branches WHERE id = ? AND project_id = ?`, branchID.Int64, projectInternalID).Scan(&branchCurrentID); err != nil || !branchCurrentID.Valid || uint64(branchCurrentID.Int64) != nodeInternalID {
	...
	if err := s.reviews.Row(ctx, `SELECT name, description, body FROM smart_contracts WHERE uuid = ?`, request.SmartContract.ID).
...
func (s *Server) loadClosureReviewScope(ctx context.Context, projectID, targetNodeID string) ([]closureReviewScopeNode, error) {
	projectInternalID, err := internalID(ctx, s.reviews, "projects", projectID)
	if err != nil {
		return nil, err
	}
	targetInternalID, err := internalID(ctx, s.reviews, "execution_contracts", targetNodeID)
	if err != nil {
		return nil, err
	}
	queue := []uint64{targetInternalID}
	visited := map[uint64]struct{}{targetInternalID: {}}
	scope := make([]closureReviewScopeNode, 0)
	for len(queue) > 0 {
		targetID := queue[0]
		queue = queue[1:]
		rows, err := s.reviews.Rows(ctx, `
...
func (s *Server) loadProjectAIKey(ctx context.Context, userID uint64, projectID string) (aiStoredKey, error) {
	stored, err := s.aiKeyStore.FindProjectReviewKey(ctx, userID, projectID)
	if err != nil {
...
func requestAIReviewFromModel(ctx context.Context, key aiStoredKey, reviewRequest reviewExecutionNodeRequest) (aiReviewModelOutput, error) {
	...
	response, err := (&http.Client{Timeout: sharedconstants.AIReviewHTTPTimeout}).Do(request)
	if err != nil {
...
func requestAIReviewFinalizationFromModel(ctx context.Context, key aiStoredKey, reviewRequest reviewExecutionNodeRequest, reasoning string) (aiReviewModelOutput, error) {
	...
	response, err := (&http.Client{Timeout: sharedconstants.AIReviewFinalizationHTTPTimeout}).Do(request)
	if err != nil {
