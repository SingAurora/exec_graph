...
func (h *Handler) reviewExecutionNode(w http.ResponseWriter, r *http.Request) {
	...
	key, err := h.loadProjectAIKey(ctx, user.ID, request.Project.ID)
	if errors.Is(err, sql.ErrNoRows) {
...
func (h *Handler) reviewExecutionNodeClarification(w http.ResponseWriter, r *http.Request) {
	user, ok := h.requireUser(w, r)
	if !ok {
		return
	}
...
func (h *Handler) expandCompletionReviewScope(ctx context.Context, request *reviewExecutionNodeRequest) error {
	closureNodes, err := h.loadClosureReviewScope(ctx, request.Project.ID, request.NodeID)
	if err != nil {
